...
test("offline acquisition creates a named task worktree from local trunk", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "treehouse-new-"));
  const repository = await createRepository(home);

  const output = await runTreehouse(repository, home, ["new", "Auth Redirect", "--offline", "--json"]);
  ...
  assert.equal(acquired.worktree.path, path.join(home, ".treehouse", "alpha", "auth-redirect"));
...
test("interactive offline acquisition opens a managed shell in the task worktree", async () => {
  ...
  await writeFile(shell, "#!/bin/sh\nprintf '%s' \"$PWD\" > \"$TREEHOUSE_TEST_MARKER\"\n");
  await chmod(shell, 0o700);

  const output = await runTreehouse(repository, home, ["new", "Shell Task", "--offline"], {
    SHELL: shell,
    TREEHOUSE_TEST_MARKER: marker,
  });

  assert.equal(output.exitCode, 0, output.stderr);
  assert.equal(await readFile(marker, "utf8"), await realpath(path.join(home, ".treehouse", "alpha", "shell-task")));
});
...
test("path output does not open an interactive shell", async () => {
  ...
  await writeFile(shell, "#!/bin/sh\nprintf opened > \"$TREEHOUSE_TEST_MARKER\"\n");
  await chmod(shell, 0o700);

  const output = await runTreehouse(repository, home, ["new", "Path Task", "--offline", "--print-path"], {
    SHELL: shell,
    TREEHOUSE_TEST_MARKER: marker,
  });

  assert.equal(output.exitCode, 0, output.stderr);
  assert.equal(output.stdout.trim(), path.join(home, ".treehouse", "alpha", "path-task"));
...
test("a newly colliding repository receives a parent-qualified project group", async () => {
  ...
  const first = await runTreehouse(firstRepository, home, ["new", "First Task", "--offline", "--json"]);

  const second = await runTreehouse(secondRepository, home, ["new", "Second Task", "--offline", "--json"]);
  ...
  assert.equal(JSON.parse(second.stdout).worktree.path, path.join(home, ".treehouse", "team-two-alpha", "second-task"));
});
...
test("invalid capacity fails before creating a project group or branch", async () => {
  ...
  const output = await runTreehouse(repository, home, ["new", "Invalid Capacity", "--offline", "--json"], {
    TREEHOUSE_MAX_TREES: "0",
  });

  assert.equal(output.exitCode, 1);
  assert.match(output.stderr, /TREEHOUSE_MAX_TREES must be a positive integer/);
...
test("concurrent acquisitions preserve both unique task slots", async () => {
  ...
  const outputs = await Promise.all([
    runTreehouse(repository, home, ["new", "Concurrent One", "--offline", "--json"]),
    runTreehouse(repository, home, ["new", "Concurrent Two", "--offline", "--json"]),
  ]);
...
test("offline acquisition refuses to branch from a non-trunk checkout", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "treehouse-new-"));
  const repository = await createRepository(home);
  const first = await runTreehouse(repository, home, ["new", "Register Trunk", "--offline", "--json"]);
  assert.equal(first.exitCode, 0, first.stderr);
  git(repository, ["switch", "-c", "temporary-base"]);

  const output = await runTreehouse(repository, home, ["new", "Second Task", "--offline", "--json"]);
...
test("offline acquisition refuses a dirty main project directory", async () => {
  ...
  const output = await runTreehouse(repository, home, ["new", "Dirty Trunk", "--offline", "--json"]);
...
test("capacity rejects acquisition without evicting existing task worktrees", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "treehouse-new-"));
  const repository = await createRepository(home);
  const environment = { TREEHOUSE_MAX_TREES: "2" };
  const first = await runTreehouse(repository, home, ["new", "First Slot", "--offline", "--json"], environment);
  const second = await runTreehouse(repository, home, ["new", "Second Slot", "--offline", "--json"], environment);

  const rejected = await runTreehouse(repository, home, ["new", "Third Slot", "--offline", "--json"], environment);

...
import { findMainProjectDirectory, runGit } from "../../treehouse/runtime/git.mjs";

test("Git gateway bounds external command duration", async () => {
  const directory = await mkdtemp(path.join(tmpdir(), "treehouse-git-"));
...
test("main project discovery resolves a submodule's real checkout from linked worktrees", async () => {
  const directory = await mkdtemp(path.join(tmpdir(), "treehouse-submodule-"));

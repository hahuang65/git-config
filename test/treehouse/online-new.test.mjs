...
test("online acquisition fast-forwards trunk before creating the task branch", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "treehouse-online-"));
  ...
  const output = await runTreehouse(repository, home, ["new", "Current Trunk", "--json"]);
...
test("online acquisition uses a trusted global sync alias", async () => {
  ...
  git(home, ["config", "--file", globalConfig, "alias.sync", "!f() { printf synced > \"$TREEHOUSE_SYNC_MARKER\" && git fetch --all --prune && git merge --ff-only '@{upstream}'; }; f"]);

  const output = await runTreehouse(repository, home, ["new", "Alias Sync", "--json"], {
    GIT_CONFIG_GLOBAL: globalConfig,
    TREEHOUSE_SYNC_MARKER: marker,
  });
...
test("online acquisition creates the task branch through a trusted global new alias", async () => {
  ...
  git(home, ["config", "--file", globalConfig, "alias.new", "!f() { printf '%s' \"$1\" > \"$TREEHOUSE_NEW_MARKER\" && git switch --no-track --create \"team/$1\" main; }; f"]);

  const output = await runTreehouse(repository, home, ["new", "Alias Branch", "--json"], {
    GIT_CONFIG_GLOBAL: globalConfig,
    TREEHOUSE_NEW_MARKER: marker,
  });
...
test("repository-local shell aliases are ignored during acquisition", async () => {
  ...
  const output = await runTreehouse(repository, home, ["new", "Safe Fallback", "--json"]);
...
test("a failing trusted sync alias stops without explicit fallback mutation", async () => {
  ...
  const output = await runTreehouse(repository, home, ["new", "Must Stop", "--json"], {
    GIT_CONFIG_GLOBAL: globalConfig,
  });
...
test("a successful sync alias must still satisfy trunk postconditions", async () => {
  ...
  const output = await runTreehouse(repository, home, ["new", "Stale Alias", "--json"], {
    GIT_CONFIG_GLOBAL: globalConfig,
  });
...
test("online acquisition resolves trunk from remote metadata instead of the current branch", async () => {
  ...
  const output = await runTreehouse(repository, home, ["new", "Wrong Trunk", "--json"]);

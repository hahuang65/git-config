...
async function writeProjectState(home, name, root) {
  const group = path.join(home, ".treehouse", name);
...
async function runTreehouse(args = [], options = {}) {
...
test("top-level help documents every command and compatibility boundary", async () => {
  const output = await runTreehouse(["--help"]);

  assert.equal(output.exitCode, 0);
  assert.equal(output.stderr, "");
  assert.match(output.stdout, /Usage: treehouse \[command\]/);
...
test("unknown commands fail with usage guidance", async () => {
  const output = await runTreehouse(["plant"]);

  assert.equal(output.exitCode, 2);
  assert.equal(output.stdout, "");
  assert.match(output.stderr, /Unknown command: plant/);
  assert.match(output.stderr, /treehouse --help/);
});

test("status help documents scope and output controls", async () => {
  const output = await runTreehouse(["status", "--help"]);

  assert.equal(output.exitCode, 0);
  assert.match(output.stdout, /Usage: treehouse status/);
...
test("every subcommand help is comprehensive and never mutates Treehouse state", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "treehouse-home-"));
  ...
  for (const [command, options] of Object.entries(expectedOptions)) {
    const output = await runTreehouse([command, "--help"], { cwd: repository, home });
    ...
    assert.match(output.stdout, new RegExp(`Usage: treehouse ${command}`));
  ...
  await assert.rejects(stat(path.join(home, ".treehouse")), { code: "ENOENT" });
});

test("bare invocation reports that an isolated home has no Treehouse projects", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "treehouse-home-"));

  const output = await runTreehouse([], { cwd: home, home });

  assert.deepEqual(output, {
    stdout: "No Treehouse projects.\n",
    stderr: "",
    exitCode: 0,
  });
});
...
test("status emits a versioned machine-readable outcome", async () => {
  ...
  const output = await runTreehouse(["status", "--json"], { cwd: home, home });
...
test("status inside a repository shows only the registered project group", async () => {
  ...
  const output = await runTreehouse(["status", "--json"], { cwd: repository, home });
...
test("status inside a linked worktree resolves its registered main project", async () => {
  ...
  const output = await runTreehouse(["status", "--json"], { cwd: linked, home });
...
test("status --all overrides repository scoping", async () => {
  ...
  const output = await runTreehouse(["status", "--all", "--json"], { cwd: repository, home });

...
import { proveLanding } from "../../treehouse/runtime/landing.mjs";
...
async function createLandedGraph() {
  const repository = await mkdtemp(path.join(tmpdir(), "treehouse-landing-"));
  git(repository, ["init", "--initial-branch=main"]);
  await writeFile(path.join(repository, "README.md"), "base\n");
  git(repository, ["add", "README.md"]);
  git(repository, ["-c", "user.name=Treehouse Test", "-c", "user.email=test@example.com", "commit", "-m", "base"]);
  git(repository, ["switch", "-c", "feature"]);
  await writeFile(path.join(repository, "feature.txt"), "feature\n");
  git(repository, ["add", "feature.txt"]);
  git(repository, ["-c", "user.name=Treehouse Test", "-c", "user.email=test@example.com", "commit", "-m", "feature"]);

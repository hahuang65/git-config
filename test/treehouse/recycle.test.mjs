...
test("recycle turns a clean landed task into a detached available slot", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "treehouse-recycle-"));
  const repository = await createRepository(home);
  const task = await createLandedTask(home, repository);

  const output = await runTreehouse(repository, home, ["recycle", "recyclable", "--json"]);
...
test("a later acquisition reuses the detached available slot", async () => {
  ...
  const recycledOutput = await runTreehouse(repository, home, ["recycle", "recyclable", "--json"]);
  assert.equal(recycledOutput.exitCode, 0, recycledOutput.stderr);
  const recycled = JSON.parse(recycledOutput.stdout).slot;

  const acquiredOutput = await runTreehouse(repository, home, ["new", "Reused Slot", "--offline", "--json"]);

  assert.equal(acquiredOutput.exitCode, 0, acquiredOutput.stderr);
  const acquired = JSON.parse(acquiredOutput.stdout).worktree;
  assert.equal(acquired.path, path.join(home, ".treehouse", "alpha", "reused-slot"));
  const state = JSON.parse(await readFile(path.join(home, ".treehouse", "alpha", "state.json"), "utf8"));
...
test("recycle preserves an unlanded task worktree and branch", async () => {
  ...
  git(task.path, ["add", "feature.txt"]);
  git(task.path, ["-c", "user.name=Treehouse Test", "-c", "user.email=test@example.com", "commit", "-m", "unlanded"]);

  const output = await runTreehouse(repository, home, ["recycle", "unlanded", "--json"]);
...
test("recycle refuses a dirty landed task", async () => {
  ...
  const output = await runTreehouse(repository, home, ["recycle", "dirty-landed", "--json"]);
...
test("recycle refuses a task with a live owner", async () => {
  ...
  const entered = await runTreehouse(repository, home, ["enter", "occupied", "--owner-pid", String(process.pid), "--json"]);
  assert.equal(entered.exitCode, 0, entered.stderr);

  const output = await runTreehouse(repository, home, ["recycle", "occupied", "--json"]);
...
test("recycle can retain the completed local branch explicitly", async () => {
  ...
  const output = await runTreehouse(repository, home, ["recycle", "keep-branch", "--keep-branch", "--json"]);
...
test("recycle accepts exact-head squash landing proven by pull-request metadata", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "treehouse-recycle-"));
  const repository = await createRepository(home);
  const created = await runTreehouse(repository, home, ["new", "Squashed", "--offline", "--json"]);
  assert.equal(created.exitCode, 0, created.stderr);
  const task = JSON.parse(created.stdout).worktree;
  ...
  git(repository, ["-c", "user.name=Treehouse Test", "-c", "user.email=test@example.com", "commit", "-m", "squash feature"]);
  ...
  const output = await runTreehouse(repository, home, ["recycle", "squashed", "--json"], {

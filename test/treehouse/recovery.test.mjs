...
test("refresh reconstructs known worktree facts from missing durable state", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "treehouse-recovery-"));
  const repository = await createRepository(home);
  const created = await runTreehouse(repository, home, ["new", "Recover Me", "--offline", "--json"]);
  assert.equal(created.exitCode, 0, created.stderr);
  const task = JSON.parse(created.stdout).worktree;
  const statePath = path.join(home, ".treehouse", "alpha", "state.json");
  await unlink(statePath);

  const output = await runTreehouse(repository, home, ["status", "--refresh", "--json"]);
...
test("refresh preserves live owners and releases only proven-dead owners", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "treehouse-recovery-"));
  const repository = await createRepository(home);
  const created = await runTreehouse(repository, home, ["new", "Owned", "--offline", "--json"]);
  assert.equal(created.exitCode, 0, created.stderr);
...
test("refresh quarantines conflicting branch registrations", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "treehouse-recovery-"));
  const repository = await createRepository(home);
  const created = await runTreehouse(repository, home, ["new", "Conflicted", "--offline", "--json"]);
  assert.equal(created.exitCode, 0, created.stderr);
  const statePath = path.join(home, ".treehouse", "alpha", "state.json");
  const state = JSON.parse(await readFile(statePath, "utf8"));
  ...
  const acquired = await runTreehouse(repository, home, ["new", "Another", "--offline", "--json"]);
...
test("acquisition reconstructs truncated state before allocating another slot", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "treehouse-recovery-"));
  const repository = await createRepository(home);
  const created = await runTreehouse(repository, home, ["new", "Existing", "--offline", "--json"]);
  assert.equal(created.exitCode, 0, created.stderr);
  const statePath = path.join(home, ".treehouse", "alpha", "state.json");
  await writeFile(statePath, "{\"version\":1,");

  const acquired = await runTreehouse(repository, home, ["new", "After Recovery", "--offline", "--json"]);
...
test("an interrupted temporary write cannot replace the last valid state", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "treehouse-recovery-"));
  const repository = await createRepository(home);
  const created = await runTreehouse(repository, home, ["new", "Atomic", "--offline", "--json"]);
  assert.equal(created.exitCode, 0, created.stderr);
  const statePath = path.join(home, ".treehouse", "alpha", "state.json");

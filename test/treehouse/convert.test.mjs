...
async function commitFile(repository, filename, content, message) {
  ...
  git(repository, ["-c", "user.name=Treehouse Test", "-c", "user.email=test@example.com", "commit", "-m", message]);
}
...
test("convert moves a clean local task branch into a managed worktree", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "treehouse-convert-"));
  ...
  const output = await runTreehouse(repository, home, ["convert", "Auth Isolation", "--json"]);
...
test("conversion failure restores the local task branch and reports the recoverable target", async () => {
  ...
  const featureTip = git(repository, ["rev-parse", "HEAD"]);
  const target = path.join(home, ".treehouse", "alpha", "blocked-target");
  await mkdir(target, { recursive: true });
  await writeFile(path.join(target, "sentinel"), "keep\n");

  const output = await runTreehouse(repository, home, ["convert", "Blocked Target", "--json"]);
...
test("conversion rejects trunk without creating Treehouse state", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "treehouse-convert-"));
  const repository = await createRemoteRepository(home);

  const output = await runTreehouse(repository, home, ["convert", "Not A Task", "--json"]);
...
test("conversion rejects detached HEAD without mutation", async () => {
  ...
  const output = await runTreehouse(repository, home, ["convert", "Detached", "--json"]);
...
test("conversion rejects a branch already attached to a linked worktree", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "treehouse-convert-"));
  const repository = await createRemoteRepository(home);
  const created = await runTreehouse(repository, home, ["new", "Already Linked", "--offline", "--json"]);
  assert.equal(created.exitCode, 0, created.stderr);
  const taskPath = JSON.parse(created.stdout).worktree.path;

  const output = await runTreehouse(taskPath, home, ["convert", "Again", "--json"]);
...
test("dirty conversion preserves staged, unstaged, and untracked state", async () => {
  ...
  const output = await runTreehouse(repository, home, ["convert", "Dirty State", "--json"]);
...
test("dirty conversion moves untracked files while leaving ignored files behind", async () => {
  ...
  const output = await runTreehouse(repository, home, ["convert", "File State", "--json"]);
...
test("restoration conflicts preserve recovery state and identify the stash", async () => {
  ...
  await writeFile(hook, "#!/bin/sh\ncase \"$PWD\" in\n  */.treehouse/*) printf 'conflicting commit\\n' > tracked.txt; git add tracked.txt; git -c user.name='Treehouse Test' -c user.email=test@example.com commit -m conflict >/dev/null ;;\nesac\n");
  await chmod(hook, 0o700);
  git(repository, ["config", "core.hooksPath", hooks]);

  const output = await runTreehouse(repository, home, ["convert", "Conflict State", "--json"]);

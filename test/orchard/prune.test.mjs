...
async function createTwoAvailableSlots(home, repository) {
  ...
  git(first.path, ["-c", "user.name=Orchard Test", "-c", "user.email=test@example.com", "commit", "-m", "first"]);
  git(repository, ["merge", "--ff-only", first.branch]);
  const recycledFirst = await runOrchard(repository, home, ["recycle", "first", "--json"]);
  const recycledSecond = await runOrchard(repository, home, ["recycle", "second", "--json"]);
...
test("prune is a complete dry run by default", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "orchard-prune-"));
  ...
  const output = await runOrchard(repository, home, ["prune", "--json"], { ORCHARD_MAX_TREES: "1" });
...
test("prune apply removes only available slots above capacity", async () => {
  ...
  const output = await runOrchard(repository, home, ["prune", "--apply", "--json"], {
    ORCHARD_MAX_TREES: "1",
  });
...
test("prune apply recycles eligible landed tasks before removing excess", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "orchard-prune-"));
  const repository = await createRepository(home);
  const landedOutput = await runOrchard(repository, home, ["new", "Landed", "--offline", "--json"]);
  const activeOutput = await runOrchard(repository, home, ["new", "Active", "--offline", "--json"]);
  ...
  git(active.path, ["-c", "user.name=Orchard Test", "-c", "user.email=test@example.com", "commit", "-m", "active"]);

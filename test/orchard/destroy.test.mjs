...
test("destroy is a complete dry run by default", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "orchard-destroy-"));
  ...
  const output = await runOrchard(repository, home, ["destroy", "--json"]);
...
test("destroy apply can remove unlanded work while retaining its local branch", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "orchard-destroy-"));
  const repository = await createRepository(home);
  const created = await runOrchard(repository, home, ["new", "Retain Branch", "--offline", "--json"]);
  assert.equal(created.exitCode, 0, created.stderr);
  const task = JSON.parse(created.stdout).worktree;
  await writeFile(path.join(task.path, "dirty.txt"), "authorized loss\n");

  const output = await runOrchard(repository, home, ["destroy", "--apply", "--allow-unlanded", "--json"]);
...
test("destroy requires every matching risk gate independently", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "orchard-destroy-"));
  const repository = await createRepository(home);
  const created = await runOrchard(repository, home, ["new", "All Risks", "--offline", "--json"]);
  assert.equal(created.exitCode, 0, created.stderr);
  const task = JSON.parse(created.stdout).worktree;
  await writeFile(path.join(task.path, "dirty.txt"), "risk\n");
  const entered = await runOrchard(repository, home, ["enter", "all-risks", "--owner-pid", String(process.pid), "--json"]);
  ...
  const blocked = await runOrchard(repository, home, ["destroy", "--apply", "--json"]);
  ...
  const allowed = await runOrchard(repository, home, [
...
test("local branch deletion requires its separate explicit option", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "orchard-destroy-"));
  const repository = await createRepository(home);
  const created = await runOrchard(repository, home, ["new", "Delete Branch", "--offline", "--json"]);
  assert.equal(created.exitCode, 0, created.stderr);
  const task = JSON.parse(created.stdout).worktree;

  const output = await runOrchard(repository, home, ["destroy", "--apply", "--delete-branches", "--json"]);
...
test("partial destroy failure keeps surviving work registered", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "orchard-destroy-"));
  const repository = await createRepository(home);
  const firstOutput = await runOrchard(repository, home, ["new", "First", "--offline", "--json"]);
  const secondOutput = await runOrchard(repository, home, ["new", "Second", "--offline", "--json"]);
  ...
  const output = await runOrchard(repository, home, [

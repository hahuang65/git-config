...
test("enter reopens a task worktree without changing its lifecycle identity", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "orchard-enter-"));
  const repository = await createRepository(home);
  const created = await runOrchard(repository, home, ["new", "Existing Task", "--offline", "--json"]);
  assert.equal(created.exitCode, 0, created.stderr);
  const task = JSON.parse(created.stdout).worktree;

  const output = await runOrchard(repository, home, ["enter", "existing-task", "--json"]);
...
test("managed shell entry releases its ownership claim when the shell exits", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "orchard-enter-"));
  const repository = await createRepository(home);
  const created = await runOrchard(repository, home, ["new", "Shell Entry", "--offline", "--json"]);
  assert.equal(created.exitCode, 0, created.stderr);
  const shell = path.join(home, "exit-shell");
  await writeFile(shell, "#!/bin/sh\nexit 0\n");
  await chmod(shell, 0o700);

  const output = await runOrchard(repository, home, ["enter", "shell-entry"], { SHELL: shell });
...
test("a live exclusive owner blocks another caller", async (context) => {
  const home = await mkdtemp(path.join(tmpdir(), "orchard-enter-"));
  const repository = await createRepository(home);
  const created = await runOrchard(repository, home, ["new", "Owned Task", "--offline", "--json"]);
  assert.equal(created.exitCode, 0, created.stderr);
  const first = await runOrchard(repository, home, ["enter", "owned-task", "--owner-pid", String(process.pid), "--json"]);
  assert.equal(first.exitCode, 0, first.stderr);
  const sleeper = spawn(process.execPath, ["-e", "setTimeout(() => {}, 30000)"]);
  context.after(() => sleeper.kill());

  const blocked = await runOrchard(repository, home, ["enter", "owned-task", "--owner-pid", String(sleeper.pid), "--json"]);
...
test("explicit shared entry records an independent live owner", async (context) => {
  const home = await mkdtemp(path.join(tmpdir(), "orchard-enter-"));
  const repository = await createRepository(home);
  const created = await runOrchard(repository, home, ["new", "Shared Task", "--offline", "--json"]);
  assert.equal(created.exitCode, 0, created.stderr);
  const first = await runOrchard(repository, home, ["enter", "shared-task", "--owner-pid", String(process.pid), "--json"]);
  assert.equal(first.exitCode, 0, first.stderr);
  const sleeper = spawn(process.execPath, ["-e", "setTimeout(() => {}, 30000)"]);
  context.after(() => sleeper.kill());

  const shared = await runOrchard(repository, home, ["enter", "shared-task", "--owner-pid", String(sleeper.pid), "--share", "--json"]);
...
test("a dead owner is cleared only after process liveness validation", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "orchard-enter-"));
  const repository = await createRepository(home);
  const created = await runOrchard(repository, home, ["new", "Stale Owner", "--offline", "--json"]);
  assert.equal(created.exitCode, 0, created.stderr);
  ...
  const output = await runOrchard(repository, home, ["enter", "stale-owner", "--owner-pid", String(process.pid), "--json"]);
...
test("machine callers can release an ownership token after session shutdown", async () => {
  const home = await mkdtemp(path.join(tmpdir(), "orchard-enter-"));
  const repository = await createRepository(home);
  const created = await runOrchard(repository, home, ["new", "Release Owner", "--offline", "--json"]);
  assert.equal(created.exitCode, 0, created.stderr);
  const entered = await runOrchard(repository, home, ["enter", "release-owner", "--owner-pid", String(process.pid), "--json"]);
  assert.equal(entered.exitCode, 0, entered.stderr);
  const ownerToken = JSON.parse(entered.stdout).owner.token;

  const released = await runOrchard(repository, home, ["enter", "release-owner", "--release-owner", ownerToken, "--json"]);

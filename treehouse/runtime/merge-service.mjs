...
export async function mergeTask({ cwd, home, intent, keep = false }) {
  ...
  if (!projectRoot) throw new Error("treehouse merge must run inside a Git repository");
...
async function resolveTaskSlot(registry, cwd, intent) {
  ...
  if (!slot) throw new Error("treehouse merge requires a managed task worktree or explicit intent");
  return slot;
}
...
async function validateMerge(registry, slot) {
  ...
  if (!await isTipAncestorOfTrunk(projectRoot, trunk, slot.branch)) {
    throw new Error(`Trunk '${trunk}' cannot fast-forward to '${slot.branch}'`);
  }
}
...
async function fastForwardTask(registry, slot, { keep }) {
  ...
  return {
    ...
    integration: { status: "fast-forwarded", tip: featureTip },

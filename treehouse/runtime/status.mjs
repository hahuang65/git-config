...
export async function readTreehouseStatus({ home = process.env.HOME, cwd = process.cwd(), all = false } = {}) {
  if (!home) throw new Error("HOME is required");
  const groups = await readProjectGroups(path.join(home, ".treehouse"));
...
export function formatTreehouseStatus(status) {
  if (status.projects.length === 0) return "No Treehouse projects.";

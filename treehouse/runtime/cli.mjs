...
import { formatTreehouseStatus, readTreehouseStatus } from "./status.mjs";

const COMMANDS = new Set(["new", "convert", "status", "enter", "merge", "recycle", "prune", "destroy"]);

const TOP_LEVEL_HELP = `Treehouse manages reusable, branch-bound Git worktrees.
...
Commands:
  ...
  merge     Fast-forward a task branch onto trunk
...
Run treehouse <command> --help for command-specific help.
`;

const STATUS_HELP = `Show managed Treehouse project groups and worktrees without mutation.

Usage: treehouse status [--all] [--refresh] [--json]
...
Usage: treehouse new <intent> [--offline] [--print-path | --json]
...
Usage: treehouse convert <intent> [--print-path | --json]
...
Usage: treehouse enter <intent> [--share] [--owner-pid <pid>] [--print-path | --json]
       treehouse enter <intent> --release-owner <token> [--json]
...
`,
  merge: `Fast-forward a managed task branch onto trunk from the main project directory.

Usage: treehouse merge [intent] [--keep] [--json]
       treehouse merge --finalize <operation-id> [--json]
...
Safety: merge is local, fast-forward-only, and never pushes; --keep skips return and cleanup.
Failure: dirty, unmanaged, occupied, or non-fast-forward tasks leave both checkouts and caller location unchanged.
`,
  recycle: `Return one clean, landed, unoccupied task worktree to the available pool.

Usage: treehouse recycle <intent> [--keep-branch] [--json]
...
Usage: treehouse prune [project] [--apply] [--json]
...
`,
  destroy: `Preview or destroy one named Treehouse project group.

Usage: treehouse destroy [project] [--apply] [risk-options] [--json]
...
export async function runTreehouseCli(args, io = console) {
  ...
  if (command === "status") {
    ...
    const status = await readTreehouseStatus({ all: args.includes("--all") });
    const output = args.includes("--json")
      ? JSON.stringify(createMachineOutcome(command, status))
      : formatTreehouseStatus(status);
    io.log(output);
    return 0;
  }
  ...
  if (command === "enter") {
    ...
    if (releaseToken) {
      ...
      io.log(args.includes("--json")
        ...
        : (released ? "Released Treehouse owner" : "Treehouse owner was already released"));
      return 0;
    }

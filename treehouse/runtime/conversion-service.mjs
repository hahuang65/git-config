...
export async function convertTaskBranch({ cwd, home, requestedIntent }) {
  ...
  if (!checkoutRoot || !projectRoot) throw new Error("treehouse convert must run inside a Git repository");
  if (checkoutRoot !== projectRoot) throw new Error("treehouse convert must run from the main project directory");
  ...
  if (!trunk) throw new Error("Cannot determine trunk from Treehouse state or remote metadata");
...
async function convertUnderLock({ home, projectRoot, trunk, branch, intent, capacity }) {
  ...
  if (registry.state.slots.length >= capacity) {
    throw new Error(`Treehouse capacity of ${capacity} reached for ${registry.state.project.name}`);
  }

...
export async function openProjectRegistry({ home, projectRoot, trunk }) {
  const treehouseRoot = path.join(home, ".treehouse");
  await mkdir(treehouseRoot, { recursive: true });
  const name = await resolveProjectName(treehouseRoot, projectRoot);
  const directory = path.join(treehouseRoot, name);
...
export async function findProjectRegistry({ home, projectRoot }) {
  ...
  let entries;
  try {
    entries = await readdir(treehouseRoot, { withFileTypes: true });
  } catch (error) {
    if (error?.code === "ENOENT") return undefined;
    throw error;
  }
  for (const entry of entries.filter((candidate) => candidate.isDirectory())) {
    const directory = path.join(treehouseRoot, entry.name);
    const statePath = path.join(directory, "state.json");
    const state = await readState(statePath);
...
async function resolveProjectName(treehouseRoot, projectRoot) {
  const basename = normalizeIntent(path.basename(projectRoot));
  if (await nameBelongsTo(treehouseRoot, basename, projectRoot)) return basename;
  const parent = normalizeIntent(path.basename(path.dirname(projectRoot)));
  const qualified = `${parent}-${basename}`;
  if (await nameBelongsTo(treehouseRoot, qualified, projectRoot)) return qualified;
  const suffix = createHash("sha256").update(projectRoot).digest("hex").slice(0, 8);
  return `${qualified}-${suffix}`;
}

async function nameBelongsTo(treehouseRoot, name, projectRoot) {
  const state = await readState(path.join(treehouseRoot, name, "state.json"));

...
export function createManagedReturnChannel() {
  ...
  return {
    token,
    requestPath: path.join(tmpdir(), `orchard-return-${token}.json`),
  };
}

export async function writeManagedReturnRequest(transition) {
  const token = process.env.ORCHARD_RETURN_TOKEN;
  const requestPath = process.env.ORCHARD_RETURN_REQUEST;
...
function validateReturnPath(requestPath, token) {
  const expected = path.join(tmpdir(), `orchard-return-${token}.json`);
